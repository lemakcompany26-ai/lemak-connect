...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const credited = await creditWallet(service, {
      userId: user.id, transactionId, type: 'deposit', amount: expected,
    ...
    if (!credited.duplicated) {
      await service.entities.Transaction.create({
        ...
        metadata: { fundingMethod: 'card', verified: true, verification: 'verify_by_reference' },
    ...
    await sendTransactionalEmail(service, {
      ...
      data: {
        amount: expected, transactionId, reference,
