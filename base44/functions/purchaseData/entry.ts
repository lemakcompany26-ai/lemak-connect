...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const { network, phoneNumber, planId, promoCode, idempotencyKey, pin } = body;
    ...
    if (response && (isProviderSuccess(response) || (response.ok && response.status && response.status < 300 && response.data))) {
      ...
      await sendTransactionalSms(service, {
        ...
        data: { service: `Data — ${planName} (${network})`, amount: payable, recipient: phoneNumber, transactionId }
      });
